...
router.post('/', requireRole('admin'), (req, res) => {
  ...
  if (settledAmt <= 0) {
    return badRequest(res, '结算金额必须大于0');
  }
  ...
  let depositBase = 0;
  if (person_type === PERSON_TYPE_WORKER) {
  ...
  const depositFromOrders = calcDepositFromOrders(currentDeposit, depositBase);
  const unsettled = calcUnsettled(totalSalary, 0, settledTotal, depositFromOrders);
  if (settledAmt > unsettled + ROUND_TOLERANCE) {
    return badRequest(res, `结算金额超出待结算余额，当前可结算：¥${unsettled.toFixed(2)}元（已扣除押金¥${depositFromOrders.toFixed(2)}）`);
  }
  ...
  try {
    ...
    const workerAfter = person_type === PERSON_TYPE_WORKER ? db.prepare('SELECT deposit, manual_deposit_base FROM config_workers WHERE name = ?').get(person_name) : null;
    ...
    const newDepositFromOrders = calcDepositFromOrders(newDeposit, newDepositBase);
...
router.post('/deposit', requireRole('admin'), (req, res) => {
  ...
  if (!worker) {
    return badRequest(res, '员工不存在');
  }

  const currentDeposit = round2(worker.deposit || 0);
  if (currentDeposit <= 0) {
    return badRequest(res, '该员工暂无押金可退');
  }
...
router.post('/reverse/:id', requireRole('admin'), (req, res) => {
  ...
  if (!record) {
    return notFound(res, '结算记录不存在');
  }
  if (record.reversed === 1) {
    return badRequest(res, '该记录已被撤销');
  }
  if (record.person_type === PERSON_TYPE_DEPOSIT_REFUND) {
    return badRequest(res, '押金退还记录不可撤销，请直接重新结算押金');
  }
  ...
  const txn = db.transaction(() => {
    ...
    if (record.person_type === PERSON_TYPE_WORKER) {
...
router.put('/record/:id', requireRole('admin'), (req, res) => {
  ...
  if (settled_amount === undefined || settled_amount === null) {
    return badRequest(res, '缺少结算金额');
  }
  
  const settledAmt = round2(settled_amount);
  if (settledAmt < 0) {
    return badRequest(res, '结算金额不能为负数');
  }
  ...
  if (record.reversed === 1) {
    return badRequest(res, '已撤销的记录无法修改');
  }
  
  if (record.person_type === PERSON_TYPE_DEPOSIT_REFUND) {
    return badRequest(res, '押金退还记录不可修改');
  }
...
router.put('/worker-unsettled', requireRole('admin'), (req, res) => {
  ...
  if (unsettledAmt < 0) {
    return badRequest(res, '未结算金额不能为负数');
  }
